...
contract("ProductManagement", accounts => {
    ...
    it("should create a zipper and store it", () =>
        {
            const serial_number = "123456"
            const part_type = "zipper"
            const creation_date = "17/06/21"
    ...
    it("should create a bag using 3 parts and store it", async () => {
        //First, create the car parts
        const serial_numbers = ["123456", "123457", "123458"]
        const part_types = ["zipper", "Buckle", "carry handle"]
        const creation_date = "17/06/21"
        let part_array = []
        ...
        //Then create the bad itself from the parts. Each part must be given as a hash that contains the wallet address and the part info.
        const serial_prod = "12345678"
        const product_type = "bad"

        result = await contract.buildProduct(serial_prod, product_type, creation_date, part_array, {from: accounts[0]})
        //Then check the bag is in storage. Again, the hash should be the one below
        ...
        //Get parts and compare to the ones used when building the bag
