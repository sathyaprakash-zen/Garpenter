...
function populateCarDetails(item) {
    console.log("Populate Bag details, get info from chain")
    console.log(item)
    //Query blockchain for data to fill element
    window.pm.methods.products(item).call({ from: window.accounts[0] }, function (error, prod_info) {
        if (error)
            ...
            document.getElementById("bag-details-address").textContent = prod_info["manufacturer"]
            document.getElementById("bag-details-serial-num").textContent = prod_info["serial_number"]
            document.getElementById("bag-details-creation-date").textContent = prod_info["creation_date"]
            ...
            window.pm.methods.getParts(item).call({ from: window.accounts[0] }, function (error, result) {
                if (error) {
                    ...
                    document.getElementById("bag-details-parts").textContent = parts_str
                            ...
                            if (result == "0x0000000000000000000000000000000000000000") {
                                ...
                                window.co.methods.addOwnership(1, item).send({ from: window.accounts[0], gas: 1000000 }, function (error, result) {
                                    if (error) {
                                        ...
                                        console.log("Bag Ownership added")
                                    }
                                })
                            }
                        }
...
function clearBagDetails() {
    document.getElementById("bag-details-address").textContent = ""
    document.getElementById("bag-details-serial-num").textContent = ""
    document.getElementById("bag-details-parts").textContent = ""
    document.getElementById("bag-details-creation-date").textContent = ""
}
...
function bagPartListManager() {
    // Select item to use on bag manufacturing
    toggleActive(this)
}

function bagListManager() {
    toggleActive(this)
    clearActiveExcept(this)

    if (this.classList.contains("active")) {
        populateBagDetails(this.textContent)
    } else {
        clearBagDetails()
    }
}
...
function getMultipleActivePart() {
    ...
    var item_list = document.getElementsByClassName("collection-item")
    for (var i = 0; i < item_list.length; i++) {
        if (item_list[i].parentElement.id == "bag-part-list" && item_list[i].classList.contains("active")) {
...
function dealerProductListManager() {
    ...
    if (this.classList.contains("active")) {
        getOwnerHistoryFromEvents('TransferProductOwnership', this.textContent).then((owners) => {
            populateOwnerDetails(owners, "bag-owners")
        })
    } else {
        clearOwnerDetails()
    }
...
export {
    toggleActive, clearActiveExcept, populateDetails, populateBagDetails, clearDetails,
    clearBagDetails, partListManager, bagPartListManager, bagListManager, addItemToList,
